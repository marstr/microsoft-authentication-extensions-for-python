...
if not sys.platform.startswith('linux'):
    ...
    #from msal_extensions.unix import LibSecret
    from msal_extensions.token_cache import UnixTokenCache

# def test_keychain_roundtrip():
#     with LibSecret() as subject:
...
def test_osx_token_cache_roundtrip():
    ...
    if not (client_id and client_secret):
        pytest.skip('no credentials present to test UnixTokenCache round-trip with.')

    test_folder = tempfile.mkdtemp(prefix="msal_extension_test_unix_token_cache_roundtrip")
    cache_file = os.path.join(test_folder, 'msal.cache')
    try:
        subject = UnixTokenCache(cache_location=cache_file)
